...
通常の`evaluate_bls.py`による採点を実行すると、`outputs/evaluation/bls/summaries/<runと同じ名前>.txt`を自動出力し、保存先を画面とrun JSONの`summary_path`に記録します。`BLSPipeline.run()`からの実行も同様です。API上限による中断・一部失敗時も、その時点の採点済み結果をまとめます。`--transcribe-only`では生成しません。

比較対象は既定で`data/人間音声採点データ.csv`です。CSVの音声名を結果の`sample_id`に対応付け、音声を1回目、2回目、…の番号順に並べて次を記録します。

- Gemini点数、模範点数、差（Gemini − 模範）、絶対差、判定不能項目数。
- 比較できた音声の平均点、平均差、平均絶対誤差（MAE）、最大絶対誤差、点数一致本数。
- 模範と不一致、または判定不能となった項目の番号・名前・両者の判定。同点でも見逃しと誤加点が相殺されている場合があります。

点数は両者とも小数1桁に丸めてから比較します。差が負なら過小評価、正なら過大評価です。CSVに正解がない音声は「比較不可」、失敗・未処理の音声は「未採点」と表示し、乖離の集計には含めません。模範点数を一律100点にはしません。

CSVはUTF-8（BOM可）で、先頭列が`BLS評価対象コール`、続く18列が項目1〜18の順の0/1、末尾2列が`成功項目数`・`点数`の形式です。列数、音声名の重複、成功項目数と点数の整合性を確認します。別ファイルは`--human-scores-csv PATH`で指定できます。既定CSVがない場合は比較なしのスコア一覧を出し、明示指定した別CSVの読み取り失敗・不正なCSVはAPI呼び出し前にエラーにします。

模範CSVはローカル集計だけに使用し、採点プロンプト・文字起こし・点数の補正には使用しません。CSVを更新しても推論キャッシュは無効になりません。TXTに比較時のCSVパスとSHA-256を記録します。

すでに採点済みの場合、APIを使わずTXTだけを作ることもできます。

```bash
uv run python scripts/benchmark_bls.py summary \
    --run outputs/evaluation/bls/runs/20260912T080745_203a5127bc81.json \
    --results-dir outputs/evaluation/bls/results/revised3 \
    --output outputs/evaluation/bls/summaries/revised3_scores_new.txt
```

`--results-dir`はJSONを移動した場合の保存先です。省略時はrunの`result_path`を使い、見つからない場合は元の保存先の配下を検索します。同じ音声の別実験と混同しないよう、ハッシュを含む完全なファイル名が一意に一致した場合だけ使用します。既存TXTとrun JSONは書き換えません。
...
| `summaries/` | 実行ごとのスコア一覧、人手採点との差、不一致項目をまとめたTXT |
